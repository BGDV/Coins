...
/*
 You have N grams of silver, from which it is planned to issue coins. For a coin, a blank weighing K​ gram is made, after which coins of M​ gram each are made from each blank (the maximum possible number from each blank). If something remains of the blanks after that, then the material is returned to the beginning of the production cycle and fused with what was left during the manufacture of the blanks. Thus, the process will stop when there are less than K grams of silver left in the alloy.
...
let input = readLine()
let separate = input!.split(separator: " ")

var N = Int(separate[0]) ?? 0
let K = Int(separate[1]) ?? 0
let M = Int(separate[2]) ?? 0
